...
class HistDataGetter(object):
    ...
    def get_mc_raw(self, column_name="mass_4l", tag="Background".lower()):
        ...
        if tag.lower() == "background":
            ...
            c_a_y = [f"mc_track_ZZ_{str(it).split('_ZZ_to_4L_to_')[-1].split('_')[0]}_{self.info[0][0]}" for it in files_]
            hist.fill_hist_from_dir(col_=column_name, dir_=None, info=self.info, name_bac_mc_=c_a_y, file_list_=files_)
        
        if tag.lower() == "signal":
            files_ = [it for it in raw_file_list if "_H_to_" in it]
            c_a_y = [f"mc_track_H_ZZ_{str(it).split('_ZZ_to_4L_to_')[-1].split('_')[0]}_{self.info[0][0]}" for it in files_]
            hist.fill_hist_from_dir(col_=column_name, dir_=None, info=self.info, name_sig_mc_=c_a_y, file_list_=files_)
