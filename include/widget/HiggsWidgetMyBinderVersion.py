...
class HiggsWidget(object):
    ...
    def plot(self, *args, **kwargs):
        ...
        for i, (num, color) in enumerate(zip(self.mc_sig_name_list, colors)):
            if kwargs[r"$m_{\mathrm{H}}$" + f" = {num} GeV"]:
                ...
                self.histograms["mc_bac"].draw(pass_name=["data", "mc_bac"],
                                               ...
                                               label=[f"{self.td['measurement'][self.la]}", 
                                                      f"{self.td['background'][self.la]}:"+r" $ZZ, Z\gamma$"],
                                               figure=fig, ax=ax)
        ...
        if kwargs[self.ui_components["select_m_option"].description] == self.td["reset measurement"][self.la]:
            print(self.td["to reset measurement type Yes"][self.la])
            if kwargs[self.ui_components["add_m_val"].description] == "Yes":
                self.ui_components["add_m_val"].value = ""
