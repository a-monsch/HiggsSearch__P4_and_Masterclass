...
class WuerfelWidget(object):
    ...
    def build_ui_components(self):
        ...
        m_show = ipw.HTML(value=self._build_table(),
                          placeholder='',
    ...
    def _build_table(self):
        ...
        _plot_ys_array = np.histogram(self.simulation_list, self.bins, density=False, range=(0.5, self.bins + 0.5))[0]
        _ys_sum = np.sum(_plot_ys_array)
        _plot_ys_array_str = ["    " if _ys_sum == 0.0 else item for item in _plot_ys_array]

        _plot_ym_array = np.histogram(self.measurement_list, self.bins, density=False, range=(0.5, self.bins + 0.5))[0]
    ...
    def build_ui(self):
        ...
        _b_info = HBox([self.ui_components["m_show"]])
    ...
    def plot(self, *args, **kwargs):
        ...
        if kwargs[self.ui_components["dropdown_simulation"].description] == f"{self.td['reset simulation'][self.la]}":
            self.simulation_list = np.array([])
            self.ui_components["dropdown_simulation"].value = f"{self.td['simulate'][self.la].capitalize()} N {self.td['times'][self.la]}:"
            clear_output(True)
        ...
        if kwargs[self.ui_components["dropdown_measurement"].description] == f"{self.td['reset measurement'][self.la]}":
            self.ui_components["dropdown_measurement"].value = f"{self.td['measure'][self.la].capitalize()} N {self.td['times'][self.la]}:"
